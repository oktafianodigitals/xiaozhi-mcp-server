...
document.getElementById("btn-test-browser")?.addEventListener("click", async () => {
  ...
  if (resultEl) resultEl.textContent = "Mencari brave.exe…";
  try {
    const r = await apiGet("/api/browser/detect");
    if (resultEl) resultEl.textContent = `✅ Ketemu: ${r.path}`;
    showToast("Brave ditemukan", "ok");
  } catch (err) {
    if (resultEl) resultEl.textContent = `❌ ${err.message}`;
    showToast(`Brave tidak ditemukan: ${err.message}`, "err");
  }
});
