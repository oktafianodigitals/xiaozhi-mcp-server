...
from mcp.tools._browser import find_brave_executable, is_termux
...
async def detect_browser():
    """Dipakai tombol 'Test deteksi' di Settings. Cabang sesuai platform:
    - Termux: cek apakah command 'termux-open-url' tersedia (paket termux-api
      + app pendamping Termux:API).
    - Windows/desktop lain: cek apakah brave.exe ketemu lewat config manual /
      lokasi umum / PATH.
    Tidak benar-benar membuka jendela browser."""
    if is_termux():
        import shutil
        found = shutil.which("termux-open-url")
        if not found:
            raise HTTPException(
                status_code=400,
                detail=(
                    "Command 'termux-open-url' tidak ditemukan. Jalankan "
                    "'pkg install termux-api' di Termux DAN install app "
                    "pendamping 'Termux:API' dari F-Droid/Play Store."
                ),
            )
        return {"ok": True, "platform": "termux", "path": found,
                "note": "Akan membuka browser DEFAULT Android, bukan Brave secara spesifik."}

    try:
        path = find_brave_executable()
        return {"ok": True, "platform": "desktop", "path": path}
